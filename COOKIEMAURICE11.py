...
# Suprimir el warning de verificaci�n de SSL
...
page_load_timeout = 15  # Reducido a 15 segundos para que sea m�s r�pido
...
def extract_cookies_and_headers(base_url):
    ...
    if final_url is not None:
        ...
        try:
            response = requests.get(final_url, verify=False, timeout=10)  # SSL verification disabled, con timeout de 10 segundos
            headers = response.headers
        except requests.exceptions.Timeout:
            print(f"Timeout al solicitar la URL {final_url}. Omitiendo esta URL.")
